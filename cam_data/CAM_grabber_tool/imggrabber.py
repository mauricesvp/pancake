...
while True:
    ...
    for addr in cameraAdds:
        ...
        imgurl = (
            "https://media.dcaiti.tu-berlin.de/tccams/"
            + addr
            + "/jpg/image.jpg??camera=1&resolution=1280x720&rotation=0&audio=0&mirror=0&fps=0&compression=50"
        )
        imgfile = (
            imgDir
            + str(ipAddrToCamId(addr))
            + "/CAM_"
            + str(ipAddrToCamId(addr))
            + str(CamId_ident(addr))
            + "_"
            + str(getTimestamp())
            + ".jpg"
        )
