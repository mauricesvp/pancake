    ...
    def update(self, det, confidences, class_ids):
        ...
        if len(det) == 0:  # normal
            ...
            for objectID in list(self.disappeared.keys()):  # normal
                ...
                if (
                    (
                        self.objects[objectID][1] < self.cfg.LANE_SEPARATOR
                        and self.objects[objectID][0] < self.cfg.DEREGISTRATION_ZONE
                    )
                    or (
                        self.objects[objectID][1] > self.cfg.LANE_SEPARATOR
                        and self.objects[objectID][0]
                        > (self.cfg.FRAME_WIDTH - self.cfg.LANE_SEPARATOR)
                    )
        ...
        if len(self.objects) == 0:  # normal
            for i in range(0, len(inputCentroids)):  # normal
                ...
                if (
                    (
                        inputCentroids[i][1] < self.cfg.LANE_SEPARATOR
                        and inputCentroids[i][0] >= self.cfg.DEREGISTRATION_ZONE
                    )
                    or (
                        inputCentroids[i][1] > self.cfg.LANE_SEPARATOR
                        and inputCentroids[i][0]
                        <= (self.cfg.FRAME_WIDTH - self.cfg.DEREGISTRATION_ZONE)
                    )
                    or self.cfg.IGNORE_REGISTRATION_ZONES
                ):
                    self.register(
                        inputCentroids[i], inputBBoxes[i], confidences[i], class_ids[i]
                    )  # normal
            ...
            for row in unusedRows:
                ...
                if (
                    (
                        objectCentroids[row][1] < self.cfg.LANE_SEPARATOR
                        and objectCentroids[row][0] < self.cfg.DEREGISTRATION_ZONE
                    )
                    or (
                        objectCentroids[row][1] > self.cfg.LANE_SEPARATOR
                        and objectCentroids[row][0]
                        > (self.cfg.FRAME_WIDTH - self.cfg.DEREGISTRATION_ZONE)
                    )
                    or (self.disappeared[objectID] > self.MAX_DISAPPEARED)
                ):
            ...
            for col in unusedCols:
                if (
                    (
                        inputCentroids[col][1] < self.cfg.LANE_SEPARATOR
                        and inputCentroids[col][0] >= self.cfg.DEREGISTRATION_ZONE
                    )
                    or (
                        inputCentroids[col][1] > self.cfg.LANE_SEPARATOR
                        and inputCentroids[col][0]
                        <= (self.cfg.FRAME_WIDTH - self.cfg.DEREGISTRATION_ZONE)
                    )
                    or self.cfg.IGNORE_REGISTRATION_ZONES
                ):
