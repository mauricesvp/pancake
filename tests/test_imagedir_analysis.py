...
def test_order(source):
    ...
    timestamps = [
        list(
            map(
                ...
                if (not path.split(".")[-1] in img_formats)
                else (".".join(path.split("/")[-1].split(".")[:2])),
                dir,
            )
        )
        for dir in files
    ...
    converted_timestamps = [list(map(convert_timestamp, dir)) for dir in timestamps]

    # GET ORDERED SET
    unique_timestamps = [list(dict.fromkeys(dir)) for dir in converted_timestamps]

    s = [""] * max([len(entries) for entries in unique_timestamps])

    # COUNT FRAMES PER STAMP
    for i, stamps in enumerate(unique_timestamps):
        for j, stamp in enumerate(stamps):
            s[j] += (
                f"[{i+1}/{len(files)}]'{stamp}': "
...
def convert_timestamp(stamp: str):
    return datetime.datetime.fromtimestamp(float(stamp)).strftime("%Y-%m-%d %H:%M:%S")
